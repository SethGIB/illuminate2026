...
void TYI::WallBatch::loadTextures(const std::string& diffuse, const std::string& normal, const std::string& arm, const std::string& grad)
{
	...
	mGradientTex = gl::Texture2d::create(loadImage(loadAsset(grad)), gl::Texture::Format().mipmap().wrapS(GL_MIRRORED_REPEAT).wrapT(GL_MIRRORED_REPEAT));
...
void TYI::WallBatch::loadShaders(const std::string& vertPath, const std::string& fragPath, const vec2& texScale)
{
	...
	mShader->uniform("uCamColorMap", 4);
	mShader->uniform("uCamDepthMap", 5);
	mShader->uniform("uContoursMap", 6);
...
{
	mBatch = gl::Batch::create(geom::Plane().origin(vec3(0, 0, 0)).normal(vec3(0, 0, 1)) >> geom::Transform(scale(vec3(1.667 * 3.0, 3.0, 1.0))), mShader);
}
