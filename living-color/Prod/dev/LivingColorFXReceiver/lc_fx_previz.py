...
class LCLed:
    ...
    def show(self, target_srf):
        fill_color = pygame.Color(self.led_color.r // 4, self.led_color.g // 4, self.led_color.b // 4)
        if self.is_active:
            fill_color = self.led_color
        pygame.draw.ellipse(target_srf, fill_color, self.bounds)
...
def setup_leds(num_x, num_y, win_w, win_h):
    ...
    for y in range(num_y):
        ...
        for x in range(num_x):
            ...
            src_rgb = pygame.Color(0,0,0)
            src_rgb.hsva = (90.0, lin_map(r_id, 0, num_x * num_y, 0.0, 100.0), 100.0, 100.0)
            led_list.append(LCLed(src_rect, src_rgb, r_id))
            r_id += r_step
    return led_list
